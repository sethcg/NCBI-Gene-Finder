<script lang="ts">
  ...
  const handleProcessing = async () => {
    if($tableHeaders.accessionColumn?.index) {
      ...
      await process($tableHeaders.accessionColumn.index, $processData.apiKey, abortController.signal)
    }
  }
  ...
  <div class={'flex flex-col h-full justify-between gap-6 '}>
    ...
    <div class="flex flex-wrap justify-between place-content-end">
      <div class="flex flex-row gap-4 justify-center items-center text-center">
        <Button class={'text-text-100 bg-success-500 p-2 rounded-md focus-within:ring-0 focus:ring-0'} on:click={handleProcessing} disabled={!$tableHeaders.accessionColumn || $processData.processing} color={"none"}>
          <div class="flex flex-row px-2 gap-2 justify-center items-center text-center">
            <CpuIcon strokeWidth={2.5} size={"18"}/>
            <span>Process</span>
          </div>
        </Button>
        <Button class={'text-text-100 bg-danger-500 p-2 rounded-md focus-within:ring-0 focus:ring-0'} on:click={handleCancel} disabled={!$processData.processing} color={"none"}>
          <div class="flex flex-row px-2 gap-2 justify-center items-center text-center">
            <span>Cancel</span>
          </div>
        </Button>
      ...
      <Button class={'text-text-100 bg-success-500 p-2 rounded-md focus-within:ring-0 focus:ring-0'} on:click={handleDownload} disabled={!$processData.processComplete || $processData.processing || $processData.downloading} color={"none"}>
        <div class="flex flex-row px-2 gap-2 justify-center items-center text-center">
