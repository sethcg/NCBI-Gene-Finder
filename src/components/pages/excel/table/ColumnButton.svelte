<script lang="ts">
  ...
  $: indicatorClass = $tableHeaders.accessionColumn?.index == columnIndex ? accessionClass : baseClass;
  ...
  const handleClick = (e: any) => {
    ...
    switch (e.target.dataset.type) {
      case 'accession':
        ...
        $tableHeaders.accessionColumn = { label: label, index: columnIndex, indicatorId: `${buttonId}_indicator` }
        if(currentIndicator) currentIndicator.className = accessionClass
        break;
      case 'include':
        // REMOVE FROM ACCESSION
        if($tableHeaders.accessionColumn?.index == columnIndex) $tableHeaders.accessionColumn = undefined
        break;
      default:
        // REMOVE FROM ACCESSION
        if($tableHeaders.accessionColumn?.index == columnIndex) $tableHeaders.accessionColumn = undefined
  ...
  <div class="flex flex-col gap-y-[2px] justify-center items-start">
    <div class="flex flex-col justify-center items-center">
      ...
      <Dropdown triggeredBy="#{buttonId}">   
        <DropdownItem data-type={'accession'} data-description={'Acc #'} defaultClass={"px-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600'"} on:click={handleClick}>Accession Version</DropdownItem>
        <DropdownItem data-type={'none'} data-description={''} defaultClass={"px-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600'"} on:click={handleClick}>None</DropdownItem>
      </Dropdown>
    </div>
