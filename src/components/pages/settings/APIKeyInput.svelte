<script lang="ts">
  ...
  let apiKey = $processData.apiKey ? crypto.DES.decrypt($processData.apiKey, cryptoKey).toString(crypto.enc.Utf8) : '';
</script>
...
<div class="flex flex-col">
  ...
  <div class="flex items-center justify-between">
    <div class="inline-flex rounded-lg shadow-sm w-full">
      ...
      <input value={apiKey} id={'api-key-input'} on:focusout={handleSetKey} placeholder={"ABCD123"} class={'block focus:outline-none focus:ring-2 focus:ring-black w-full rtl:text-right p-2.5 bg-secondary-400 text-text-100 placeholder-text-400 text-sm rounded-e-lg'}/>
    </div>
  </div>
  <p id="helper-text-explanation" class="mt-2 ml-1 text-sm text-text-400 select-none">
    For more information on how to optain an API key use this <a href={undefined} on:click={openAPILink} class="font-medium text-blue-600 hover:underline dark:text-blue-500 select-auto">link</a>.
  </p>
</div>
