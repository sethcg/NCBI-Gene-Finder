...
export async function ePost(signal: AbortSignal, database: string, ids: string[], apiKey?: string, rettype?: string, links?: link[]): Promise<postResponse> {
  return new Promise<postResponse>(async (resolve, reject) => {
    if(apiKey) {
      api_limit((): Promise<postResponse> => _ePost(signal, database, ids, apiKey, rettype, links))
        .then((postResponse) => { resolve(postResponse) })
        .catch(() => { 
          handleError() 
          reject()
        })
...
async function _ePost(signal: AbortSignal, database: string, ids: string[], apiKey?: string, rettype?: string, links?: link[]): Promise<postResponse> {
...
export async function eLink(signal: AbortSignal, databaseFrom: string, databaseTo: string, ids: string[], apiKey?: string): Promise<link[]> {
...
async function _eLink(signal: AbortSignal, databaseFrom: string, databaseTo: string, ids: string[], apiKey?: string): Promise<link[]> {
  const form = new FormData()
...
export async function eSummary(signal: AbortSignal, database: string, webEnv: string, queryKey: string, apiKey?: string, count?: number, links?: link[]): Promise<summaryResponse> {
  return new Promise<summaryResponse>(async (resolve, reject) => {
    if(apiKey) {
      api_limit((): Promise<summaryResponse> => _eSummary(signal, database, webEnv, queryKey, apiKey, count, links))
        .then((summaryResponse) => { resolve(summaryResponse) })
        .catch(() => {
          reject(handleError())
        });
    } else {
      limit((): Promise<summaryResponse> => _eSummary(signal, database, webEnv, queryKey, undefined, count, links))
        .then((summaryResponse) => { resolve(summaryResponse) })
        .catch(() => {
          reject(handleError())
        });
...
async function _eSummary(signal: AbortSignal, database: string, webEnv: string, queryKey: string, apiKey?: string, count?: number, links?: link[]): Promise<summaryResponse> {
  const form = new FormData()
  form.append('db', database)
...
export async function handleError(msg?: string): Promise<void> {
  let event: processEvent = { type: 'error', message: 'Error while processing the spreadsheet.', percentage: 0 }
