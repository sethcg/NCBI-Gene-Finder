...
export async function process(accessionVersionColumnIndex: number, encrypted_apiKey: string | undefined, signal: AbortSignal): Promise<void> {
  const startTime = performance.now()
  
  // GET API KEY FROM STORED DATA (IF ANY)
  let apiKey = undefined
  if(encrypted_apiKey) apiKey = crypto.DES.decrypt(encrypted_apiKey, cryptoKey).toString(crypto.enc.Utf8);

  const accessionVersions: string[] = await getAccessionVersions(accessionVersionColumnIndex)
  let rows: row[] = accessionVersions.map((accessionVersion, index) => {return {index, accessionVersion}})
  ...
  return new Promise<void>(async () => {
    ...
    await processProteinData(rows, signal, apiKey)
      .then(async (values: row[]) => {
        await processReplacedProteinData(values, signal, apiKey)
          .then(async (values: row[]) => {
            await processGeneData(values, signal, apiKey)
              .then(async () => {
                ...
                await writeRowsToOutput(values).then(async () => {
                  ...
                  // SEND FINISHED EVENT
                  let event: processEvent = { type: 'done', message: 'Complete', percentage: 100, label: 'Processing Complete.' }
...
async function writeRowsToOutput(rows: row[]) {
  ...
  orderedRows.forEach((row: row) => {
    ...
    rowData.push(row.replaced?.protein?.accessionVersion ? row.replaced?.protein?.accessionVersion.trim() : "")
    rowData.push(row.replaced?.protein?.description ? row.replaced?.protein?.description.trim() : "")
  ...
  const outputData: Uint8Array = XLSX.write(workbook, { type: "array", bookType: "xlsx", cellStyles: true });
...
async function validateRows(rows: row[]): Promise<boolean> {
  return new Promise<boolean>(async (resolve, reject) => {
    ...
    let event: processEvent = { type: 'error', message: 'The provided row contains no accession versions.', percentage: 0, label: 'No accession versions found in selected row.' }
    await emit('process_event', event);
  }) as Promise<boolean>;
}
...
async function setProgress(addedProgress: number, progressLabel?: string): Promise<void> {
  ...
  let event: processEvent = { type: 'progress', message: '', percentage: percentage, label: progressLabel }
  await emit('process_event', event);
}
...
async function getProteinData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<row[]> {
  return new Promise<row[]>(async (resolve) => {
    await ePost(signal, 'protein', rows.map((row: row) => row.accessionVersion), apiKey, 'acc')
      .then(async (postResponse) => {
        // PROTEIN DATA POST COMPLETE: SET PROGRESS
        await setProgress(rows.length);

        await eSummary(signal, 'protein', postResponse.webEnv, postResponse.queryKey, apiKey, rows.length)
...
async function processProteinData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<row[]> {
  return new Promise<row[]>(async (resolve) => {
    ...
    const promises = batches.map(rows => new Promise<void>(async (resolve) => {
      await getProteinData(rows, signal, apiKey)
        .then(async (values: row[]) => {
          ...
          let event: processEvent = { type: 'progress', message: '', label: `Completed Batch: ${++batchNumCurrent_Protein} of ${batchNumTotal_Protein}...`}
          await emit('process_event', event);
          resolve() // BREAK OUT OF THE PROMISE
        })
      }));
    await Promise.all(promises).then(() => {resolve(rows)})
  })
}
...
async function getReplacedProteinData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<row[]> {
  return new Promise<row[]>(async (resolve) => {   
    // GET THE REPLACEDBY PROTEIN IDS
    let accessionVersions = rows.filter((row) => row.replaced).map((row: row) => row.replaced?.accessionVersion) as string[]

    // GET THE REPLACEDBY PROTEIN DATA
    await ePost(signal, 'protein', accessionVersions, apiKey, 'acc')
      .then(async (postResponse) => {
        ...
        await eSummary(signal, 'protein', postResponse.webEnv, postResponse.queryKey, apiKey, rows.length)
          .then(async (summaryResponse) => {
            ...
            proteins.forEach((protein: protein) => {
              ...
              let row = rows.find((row) => row.replaced?.accessionVersion.replace(/\..*$/, '') == protein.accessionVersion.replace(/\..*$/, ''))
              if(row?.replaced) {
...
async function processReplacedProteinData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<row[]> {
  return new Promise<row[]>(async (resolve) => {
    ...
    const promises = batches.map(rows => new Promise<void>(async (resolve) => {
      await getReplacedProteinData(rows, signal, apiKey)
        .then((values: row[]) => {
...
async function getGeneData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<row[]> {
  return new Promise<row[]>(async (resolve) => {
    ...
    let proteinIds: string[] = rows.filter((row) => row.replaced)
      .map((row) => row.replaced?.protein?.id.toString()) as string[]

    // GET ALL OTHER PROTEIN IDS
    proteinIds = proteinIds.concat(rows.filter((row) => !row.replaced)
      .map((row) => row.protein?.id.toString()) as string[])

    // GET THE PROTEIN TO GENE LINKS
    await eLink(signal, 'protein', 'gene', proteinIds, apiKey)
      .then(async (links: link[]) => {
        ...
        await ePost(signal, 'gene', geneIds, apiKey, undefined, links)
          .then(async (postResponse) => {

            // GENE DATA POST COMPLETE: SET PROGRESS
            await setProgress(rows.length);
            
            await eSummary(signal, 'gene', postResponse.webEnv, postResponse.queryKey, apiKey, undefined, postResponse.links)
              .then(async (summaryResponse) => {
...
async function processGeneData(rows: row[], signal: AbortSignal, apiKey?: string): Promise<void> {
  return new Promise<void>(async (resolve) => {
    ...
    const promises = batches.map(rows => new Promise<void>(async (resolve) => {
      await getGeneData(rows, signal, apiKey)
        .then(async (values: row[]) => {
          values.forEach((value, index) => {
            rows[index] = value
          });
          ...
          let event: processEvent = { type: 'progress', message: '', label: `Completed Batch: ${++batchNumCurrent_Gene} of ${batchNumTotal_Gene}...`}
          await emit('process_event', event);
          resolve() // BREAK OUT OF THE PROMISE
        })
      }));
